...
export async function getValidAccessToken() {
  ...
  const exp = localStorage.getItem("access_exp"); // store this when you first log in
  ...
  if (token && exp && Date.now() > parseInt(exp, 10)) {
    ...
    if (res.ok) {
      ...
      localStorage.setItem("access_exp", Date.now() + expires_in * 1000);
    } else {
      // handle failure – maybe log out
    }
  }
